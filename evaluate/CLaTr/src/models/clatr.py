...
from src.models.temos import TEMOS
from src.training.losses import InfoNCE_with_filtering
...
class CLaTr(TEMOS):
    ...
    def compute_loss(self, batch: Dict, return_all=False) -> Dict:
        ...
        if "human_feat" in batch:
            ref_human = batch["human_feat"]
            ref_feats = torch.cat([ref_traj, ref_human], dim=-1)
        else:
    ...
    def compute_loss_eval(self, batch: Dict, return_all=False) -> Dict:
        m_ref_traj = batch["traj_ref_feat"]
        m_ref_feats = m_ref_traj.clone()
        m_ref_inputs = {"x": m_ref_feats, "mask": batch["padding_mask_ref"]}

        m_pred_traj = batch["traj_pred_feat"]
        m_pred_feats = m_pred_traj.clone()
        m_pred_inputs = {"x": m_pred_feats, "mask": batch["padding_mask_pred"]}

        sent_token = batch["caption_raw"]["token"]
        t_inputs = batch["caption_feat"]
        mask = batch["padding_mask_ref"]

        # text -> traj
        t_trajectories, t_latents, t_dists = self(t_inputs, mask=mask, return_all=True)
        # traj -> traj
        m_ref_trajectories, m_ref_latents, m_ref_dists = self(m_ref_inputs, mask=mask, return_all=True)
        m_pred_trajectories, m_pred_latents, m_pred_dists = self(m_pred_inputs, mask=mask, return_all=True)
        ...
        # fmt: off
        losses["recons"] = (
            + self.reconstruction_loss_fn(t_trajectories, m_ref_feats)  # text -> traj
            + self.reconstruction_loss_fn(m_ref_trajectories, m_ref_feats)  # traj -> traj
        )
        # fmt: on
        ...
        if self.vae:
            # Create a centred normal distribution to compare with
            # logvar = 0 -> std = 1
            ref_mus = torch.zeros_like(m_ref_dists[0])
            ref_logvar = torch.zeros_like(m_ref_dists[1])
            ref_dists = (ref_mus, ref_logvar)

            losses["kl"] = (
                self.kl_loss_fn(t_dists, m_ref_dists)  # text_to_traj
                + self.kl_loss_fn(m_ref_dists, t_dists)  # traj_to_text
                + self.kl_loss_fn(m_ref_dists, ref_dists)  # traj
                + self.kl_loss_fn(t_dists, ref_dists)  # text
            )

        # Latent manifold loss
        losses["latent"] = self.latent_loss_fn(t_latents, m_ref_latents)

        # TMR: adding the contrastive loss
        losses["contrastive"] = self.contrastive_loss_fn(
            t_latents, m_ref_latents, sent_token
        )
        ...
        # Used for the validation step
        if return_all:
            return losses, t_latents, m_ref_latents, m_pred_latents, t_trajectories, m_ref_trajectories, m_pred_trajectories
    ...
    def predict_step(self, batch: Dict, batch_idx: int) -> Tensor:
        ...
        batch["m_ref_trajectories"] = m_ref_trajectories
        batch["m_pred_trajectories"] = m_pred_trajectories

        batch["ref_matrices"] = torch.stack(
            [self.get_matrix(x.permute(1, 0)) for x in batch["traj_ref_feat"][:, :, :9]]
        )
        batch["t_matrices"] = torch.stack(
            [self.get_matrix(x.permute(1, 0)) for x in t_trajectories[:, :, :9]]
        )
        batch["m_ref_matrices"] = torch.stack(
            [self.get_matrix(x.permute(1, 0)) for x in m_ref_trajectories[:, :, :9]]
        )
        batch["m_pred_matrices"] = torch.stack(
            [self.get_matrix(x.permute(1, 0)) for x in m_pred_trajectories[:, :, :9]]
        )
    ...
    def on_test_start(self):
        ...
        self.test_step_t_latents = []
        self.test_step_m_latents = []
        self.test_step_sent_token = []
    ...
    def test_step(self, batch: Dict, batch_idx: int) -> Tensor:
        ...
        # Store test values
        self.test_step_t_latents.append(t_latents)
        self.test_step_m_latents.append(m_latents)
        self.test_step_sent_token.append(batch["caption_raw"]["token"])

        return losses["loss"]

    def on_test_epoch_end(self):
        t_latents = torch.cat(self.test_step_t_latents)
        m_latents = torch.cat(self.test_step_m_latents)
        sent_tokens = torch.cat(self.test_step_sent_token)

        # Compute the similarity matrix
        sim_matrix = get_sim_matrix(t_latents, m_latents).cpu().numpy()

        self.test_contrastive_metrics = all_contrastive_metrics(
            sim_matrix,
            emb=sent_tokens.cpu().numpy(),
            threshold=self.threshold_selfsim_metrics,
        )

        self.test_step_t_latents.clear()
        self.test_step_m_latents.clear()
        self.test_step_sent_token.clear()
