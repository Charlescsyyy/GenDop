...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--keyname", "-kn", type=str, default="Movement")
...
def load_caption(json_path: str, keyname):
    """Load a txt file."""
...
def save_feats(
    ...
    for feat, sample_path in zip(feats, sample_paths):
        output_path = output_dir / Path(sample_path).parent.parent.stem / Path(sample_path).parent.stem / (Path(sample_path).stem + ".npy")
...
def extract_clip(
    ...
    with PROGRESS:
        ...
        for caption_path_batch in caption_loader:
            caption_raws = [load_caption(x, keyname) for x in caption_path_batch]
